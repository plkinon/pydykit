...
def interpolate_system(system_n, system_n1, interpolation_value):
    ...
    return system_n.copy(
        state=np.multiply(system_n.state, (1 - interpolation_value))
        + np.multiply(system_n1.state, interpolation_value)
    )
